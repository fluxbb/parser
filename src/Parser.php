		...
		switch ($tagname) {
			case 'img': // Handle bad image url, file too big, then scale-to-fit within forum defaults if too large.
				if ($tag['depth'] === 1) { // Check if not overly nested?
					if (($this->pd['ipass'] === 2) && $this->pd['config']['valid_imgs'] && url_valid($contents)) { // Valid URI?
...
{ // Error #14: 'Unable to determine remote file size.'.
	...
	function preparse_bbcode($text, &$errors, $is_signature = false)
	{
		...
		$this->pd['ipass'] = 1;
		$newtext = preg_replace_callback($this->pd['re_bbcode'], '_preparse_bbcode_callback', $text);
		...
		for ($i = 0, $len = count($parts); $i < $len; ++$i) { // Loop through hidden and non-hidden text chunks.
			...
			if ($part[0] !== "\2") { // If not hidden, process this normal text content.
				...
				$part = preg_replace_callback($this->pd['re_bbtag'], '_orphan_callback', $part);
		...
		// Process textile syntax tag shortcuts.
				if ($this->pd['config']['textile']) {
					...
					$part = preg_replace_callback($this->pd['re_textile'], '_textile_phrase_callback', $part);
					// Do lists.
		...
		$text = preg_replace_callback($this->pd['re_bbcode'], '_preparse_bbcode_callback', $text);
	...
	{
		// TODO: $this->pd
		$matches[2] = preg_replace_callback($this->pd['re_textile'], '_textile_phrase_callback', $matches[2]);
		switch ($matches[1]) {
			case '_':
				return (isset($this->pd['bbcd']['i'])) ? '[i]' . $matches[2] . '[/i]' : $matches[0];
			case '*':
				return (isset($this->pd['bbcd']['b'])) ? '[b]' . $matches[2] . '[/b]' : $matches[0];
			case '@':
				return (isset($this->pd['bbcd']['tt'])) ? '[tt]' . $matches[2] . '[/tt]' : $matches[0];
			case '^':
				return (isset($this->pd['bbcd']['sup'])) ? '[sup]' . $matches[2] . '[/sup]' : $matches[0];
			case '~':
				return (isset($this->pd['bbcd']['sub'])) ? '[sub]' . $matches[2] . '[/sub]' : $matches[0];
			case '-':
				return (isset($this->pd['bbcd']['del'])) ? '[del]' . $matches[2] . '[/del]' : $matches[0];
			case '+':
				return (isset($this->pd['bbcd']['ins'])) ? '[ins]' . $matches[2] . '[/ins]' : $matches[0];
		}
	}
	...
	*********************************************************** */
	public function _parse_bbcode_callback($matches) {
		...
		// Recursively parse any nested BBCode tag markup (unless tag type is hidden).
		if ($tag['tag_type'] !== 'hidden' && strpos($contents, '[') !== FALSE) {
			$contents = preg_replace_callback($this->pd['re_bbcode'], '_parse_bbcode_callback', $contents);
		}
		// ------------------------------------------------------------------------------
		...
		if (!$matches[2]) {	// No attribute specified? Use the NO_ATTRIB handler.
			...
			if (isset($tag['handlers']['NO_ATTRIB']))			// If we have one, use this tags
				$handler =& $tag['handlers']['NO_ATTRIB'];		// no-attribute handler. Otherwise...
			else $handler =& $this->pd['bbcd']['_ROOT_']['handlers']['NO_ATTRIB']; // Missing attribute! Strip tag.
			...
			case 'width_height': // This attribute type is used for video BBCodes (YouTube, Vimeo, ...).
				if ($attribute) {
					...
					if (isset($width)) { // If set, clip to max allowed. Convert zero to global default.
						if ($width > $this->pd['config']['max_width']) $width = $this->pd['config']['max_width'];
						elseif ($width === 0) $width = $this->pd['config']['def_width']; // If zero, set to default.
					}
					if (isset($height)) { // If set, clip to max allowed. Convert zero to global default.
						if ($height > $this->pd['config']['max_height']) $height = $this->pd['config']['max_height'];
						elseif ($height === 0) $height = $this->pd['config']['def_height']; // If zero, set to default.
					}
		...
		switch($tagname) {
			...
			case 'img': // Handle disabled image, image inside of QUOTE, and width/height dimensions in attribute.
				...
				{ // IMG not enabled in this context. Convert to a text URL link if possible and re-enable.
					if (isset($this->pd['bbcd']['url']) && $this->pd['bbcd']['url']['depth'] > 0) $format = "{%c_str%}";
					else {
			...
			case 'tr': // Clean whitespace cruft bordering <td> tags.
				...
				if (!isset($this->pd['code_scripts'])) $this->pd['code_scripts'] = array();
